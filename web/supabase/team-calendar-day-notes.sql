...
-- If saves fail with updated_by FK to `profiles`, run `team-calendar-day-notes-updated-by-fk-fix.sql`
-- (app stores auth user id; FK should target auth.users, or ensure a profiles row per user — see TeamCalendarInner upsert).
