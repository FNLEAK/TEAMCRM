// @ts-nocheck — Ballpit port (Kevin Levron / soju22); BallpitProps exported for callers.
"use client";
/**
 * Ballpit — inspired by Kevin Levron (https://x.com/soju22/status/1858925191671271801)
 * Uses RoomEnvironment + MeshPhysicalMaterial for visible, lit glass spheres.
 */
...
class x {
  ...
  #p() {
    ...
    // Three.js probes highp/mediump via getShaderPrecisionFormat().precision; on some
    // drivers that call returns null and crashes. "lowp" skips those probes.
    if (e.precision === undefined) e.precision = "lowp";
    try {
      this.renderer = new s(e);
    } catch (err) {
      console.warn("Ballpit: WebGLRenderer failed", err);
      return;
    }
  ...
  #g() {
    ...
    // Start rendering immediately; intersection/visibility observers can still pause later.
    this.#s = true;
    this.#w();
  }
  #y() {
...
class Y extends c {
  constructor(e) {
    super(e);
    // Keep Ballpit shader path conservative for broader GPU/driver compatibility.
  }
}

const X = {
  count: 200,
  colors: [0x00ff88, 0x22d3ee],
...
function createBallpit(canvas, t = {}) {
  const i = new x({
    canvas,
    size: "parent",
    maxPixelRatio: typeof window !== "undefined" ? Math.min(window.devicePixelRatio, 2) : 2,
...
export type BallpitProps = {
  ...
  friction?: number;
  wallBounce?: number;
  /** Hex colors (e.g. 0x00ff88) — at least two for gradient */
  colors?: number[];
};
...
const Ballpit = ({
  ...
  useEffect(() => {
    ...
    const initFailed = !spheresInstanceRef.current?.spheres;
    let retryTimer: number | null = null;
    if (initFailed) {
      // Some GPUs/drivers fail transiently; retry once shortly after mount.
      retryTimer = window.setTimeout(() => setRetryTick((n) => n + 1), 450);
    }
  ...
  }, [followCursor, retryTick, count, gravity, friction, wallBounce, colors]);

  useEffect(() => {
    const onVisible = () => {
      if (!document.hidden && !spheresInstanceRef.current?.spheres) {
        setRetryTick((n) => n + 1);
      }
    };
